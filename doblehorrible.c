...
double V(double r,double m, double w,  double k){
  double res = 0.5*m*w*w*r*r;
  res+= -k/r;
  return(res);
}
...
void main(int argc, char *argv[]){
  ...
  for(i=0;i<N;i++){
    ...
    a[i]=2/(h*h) + V(-Z+epsilon+(i+1)*h, masa, freq, bohr);
    c[i]=-1/(h*h);
    e[i]=-1/(h*h);
